...
class AuthentricsCallback(TrainerCallback):
    def __init__(self, project_name: str, save_stats_local: bool, *features):
        ...
        if self.project is None:
            log.info("Project not found, Creating new project")
            ...
            log.info(
                f"Found project with project name: {self.project_name} Current analysis of all checkpoints"
            )
            for file in files:
                log.info(
                    f"File Name: {file['file_name']} || Weight Contr: {file['total_weight_contribution']} || Bias Contr: {file['total_bias_contribution']}"
                )
    ...
    def on_save(
        ...
        **kwargs,
        ...
        log.info(f"Uploading checkpoint artifacts in {ckpt_dir}...")
        ...
        files = self.project["file_list"]
        if len(files) < 2:
            log.info("1st checkpoint, not running the static analysis")
            return

        log.info(f"Running Static Analysis for file: {files[-1]['file_name']}")
        ...
        log.info("Summary status of the current saved checkpoint...")
        log.info(f"Weight Summary Score: {static_analysis['weight_summary_score']}")
        log.info(f"Bias Summary Score: {static_analysis['bias_summary_score']}")
        ...
        for feature in self.features:
            if feature in static_analysis:
                ...
                log.error(f"Feature: {feature} not available for response")
        ...
        log.info(f"Created analysis response with file name: {file_name}")
        ...
        if self.token_file_path.exists():
            with self.token_file_path.open("r", encoding="utf-8") as file:
                ...
                if "token" not in data or "url" not in data:
                    log.error("Invalid token file, Please login again")
    ...
    def _check_token_validity(self, data: dict[str, str]) -> AuthentricsClient:
        try:
            ...
            log.error("Expired token. Please login again")
